...
function get20s(array){
  for (i = 0; i < array.length; i++){
    ...
    } else if (array[i].years >= "1900" && array[i].years <= "2000"){
      rangeArray.push(array[i].name)
    } else {}
  }
  return rangeArray
}
...
addArtist(artists, 20, "Corey Power", "1990 - 2020", "Web Design", "American", "I am a Web Dev student at Lambda School. I love to code!")
...
function lotsOfArt(arr){
  for (i = 0; i < arr.length; i++){
    if (arr[i].paintings > 100){
      lotsOfArtArray.push(arr[i].name)
    } else {      
    }
  }
